...
class PageController:
    ...
    async def navigate(self, url: str, wait_until: str = "load", timeout: int = 30000) -> bool:
        """
    ...
    async def wait_for_element(self,
        ...
        try:
            ...
            self.logger.error(
                f"Error waiting for element {selector}: {str(e)}")
            return None
    ...
    async def fill_form(self,
        ...
        for selector, value in form_data.items():
            try:
                ...
                if not element:
                    self.logger.warning(
                        f"Could not find form field: {selector}")
                    success = False
                    continue
    ...
    async def get_elements_text(self,
        ...
        try:
            ...
            self.logger.error(
                f"Failed to get elements text for {selector}: {str(e)}")
            return []
        ...
        try:
            ...
            if url_pattern:
                ...
                if not re.search(url_pattern, current_url):
                    self.logger.warning(
                        f"URL after navigation ({current_url}) doesn't match pattern: {url_pattern}")
                    return False
